...
class Stone(object):
    ...
    def __init__(self,x=None, y=None,direction=None,speed=None, rspeed=None):
        ...
        self.x = x if x is not None else random.randint(600, window.width)
        self.y = y if y is not None else random.randint(600, window.height)
    ...
    def tick(self, dt):
        ...
        #když kámen narazí do lodičky tak se zobrazí v okně game over
        if abs(self.x-lod.x)< self.image.width / 2 and abs(self.y-lod.y) < self.image.height /2:
            konec=pyglet.text.Label('GAME OVER',font_size=100, x=window.width//2, y=window.height//2,
